...
if (gradeQuizBtn && quizResultEl) {
  gradeQuizBtn.addEventListener("click", () => {
    ...
    quizQuestions.forEach((q, questionIndex) => {
      ...
      if (!checked) {
        allAnswered = false;
        feedbackItems.push(`Q${questionIndex + 1}: Answer this one to see the explanation.`);
        return;
      }
      const choice = parseInt(checked.value, 10);
      if (choice === q.correctIndex) {
        correctCount++;
        feedbackItems.push(`Q${questionIndex + 1}: ✅ Correct — ${q.explanation}`);
      } else {
        feedbackItems.push(`Q${questionIndex + 1}: ❌ ${q.explanation}`);
      }
    });
    ...
    if (!allAnswered) {
      ...
      details.className = "explanation-list";
      feedbackItems.forEach(item => {
        const li = document.createElement("li");
        li.textContent = item;
        details.appendChild(li);
      });
      quizResultEl.appendChild(details);
      return;
    }
