...
function openInfoModal(source) {
  if (!modalBackdrop || !modalContent || !source) return;
  modalContent.innerHTML = "";
  const clone = source.cloneNode(true);
  const cloneInputs = clone.querySelectorAll("input[id]");
  cloneInputs.forEach((input) => input.removeAttribute("id"));
...
if (modalBackdrop) {
  modalBackdrop.addEventListener("click", (event) => {
    const withinModal = event.target.closest(".card-modal");
    const clickedClose = event.target.closest(".card-modal-close");
    if (!withinModal || clickedClose) {
      closeInfoModal();
    }
  });
}
...
const infoCards = document.querySelectorAll(".card, .career-card");
