...
const totalPhishingExamples = phishingExamples.length;
...
if (nextPhishingBtn) {
  if (getCompletionStatus(activityId)) {
    ...
    nextPhishingBtn.classList.add("disabled");
    nextPhishingBtn.setAttribute("aria-disabled", "true");
    nextPhishingBtn.setAttribute("tabindex", "-1");
  }
}
  ...
  phishingButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (guess === correct) {
        ...
        if (correctGuesses === totalPhishingExamples) {
          if (nextPhishingBtn) {
            nextPhishingBtn.classList.remove("disabled");
            nextPhishingBtn.removeAttribute("aria-disabled");
            nextPhishingBtn.removeAttribute("tabindex");
            saveCompletionStatus(activityId, true);
            phishingFeedbackEl.textContent += " You've mastered phishing detection! Next challenge unlocked!";
          }
        }
      } else {
        phishingFeedbackEl.textContent = "Not quite, let's learn why. " + explanation;
        phishingFeedbackEl.className = "result bad";
        // If they get it wrong, reset correct guesses for this example to make them try again
        correctGuesses = Math.max(0, correctGuesses - 1); // Prevent negative counts
      }
...
if (newEmailBtn) {
  newEmailBtn.addEventListener("click", () => {
    currentPhishingIndex = (currentPhishingIndex + 1) % phishingExamples.length;
    showPhishingExample(currentPhishingIndex);
    // Re-enable buttons for the new example
    phishingButtons.forEach(btn => btn.disabled = false);
  });
